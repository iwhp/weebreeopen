namespace WeebreeOpen.VisualStudioServerLib.Infrastructure.Test
{
    ...
    public class VsoClient2
    {
        String baseUrl = "https://{0}.visualstudio.com/DefaultCollection/_apis/";

        // Get the alternate credentials that you'll use to access the Visual Studio Online account.
        readonly String username = "";
        readonly String password = "";
        readonly String vsoTenantName = "";
        //Api version query parameter
        readonly String apiVersion = "?api-version=1.0";

        public VsoClient2(string vsoAccountName, string userName, string password)
        {
            this.username = userName;
            this.password = password;
            this.vsoTenantName = vsoAccountName;
        }
        ...
        public async void RunSample()
        {
            ...
            baseUrl = String.Format(baseUrl, vsoTenantName);
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                responseBody = await GetAsync(client, baseUrl + "build/definitions");
                ...
                if (buildDefinitions.Value.Count() > 0)
                {
                    ...
                    responseBody = await QueueBuildAsync(client, buildRequestPostData, baseUrl + "build/requests");
        ...
        private async Task<String> QueueBuildAsync(HttpClient client, BuildRequest data, String apiUrl)
        {
            var responseBody = String.Empty;
            ...
            try
            {
                using (HttpResponseMessage response = client.PostAsync(apiUrl + apiVersion, content).Result)
                {
                    response.EnsureSuccessStatusCode();
                    responseBody = await response.Content.ReadAsStringAsync();
                }
